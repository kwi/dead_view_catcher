...
This might be really useful for big Rails app maintained by a big team.

Works with Rails 2.3 (Will be tested against Rails3 soon)

== Usage

Simply load the gem in your rails app with your favorite way.

Then instantiate DeadViewCatcher at boot:

  DeadViewCatcher.watch!

Then at any time, you just need to send a TRAP signal to your rails instance in order to generate a dump of your views status:

  kill -s TRAP pid_of_your_instance
  
If you want to generate a consolidated dump gathering data from all your instances, just choose one instance and send it the USR2 signal:
